const PitchController = pc.createScript('pitchController');

PitchController.attributes.add('ballTemplate', {
    title: 'Ball Template',
    type: 'asset',
});

PitchController.attributes.add('view', { type: 'entity' });
PitchController.attributes.add('ui', { type: 'entity' });

PitchController.attributes.add('weightCurve', {
    title: 'Weight Curve',
    type: 'json',
    schema: [{
        name: "linear",
...
PitchController.attributes.add('coefficient', {
    type: 'json',
    schema: [{
        name: "linear",
        type: 'number',
...
PitchController.attributes.add('config', {
    type: 'json',
    title: 'Configuration',
    schema: [{
        name: "historyNum",
...
PitchController.prototype.initialize = function() {    
    this.device = this.app.graphicsDevice;
    
    // Get the Screen Ratio and Reference Dimension
    const getScrRatio = _device => {
        console.log(_device);
        let refDim = (_device.width + _device.height) / 2 * _device.maxPixelRatio;
        return { width : refDim / _device.width, height : refDim / _device.height };
    };
    this.screenRatio = getScrRatio(this.device);
    if(this.app.debug) console.log(this.screenRatio);
    this.device.on('resizecanvas', () => this.screenRatio = getScrRatio(this.device), this);
    ...
    this.inputVelocity = { linear : new _inputVelocity(), angular : new _inputVelocity() };
    ...
    Object.defineProperty(this.mouseHistory, 'pushDelta', {
        ...
        value: function ( dx, dy ) {
            ...
            for (let i = 0; this.length-1 > i; ++i) this.accel.vectors[i].sub2(this.delta.vectors[i], this.delta.vectors[i+1]);
            this.accel.update();
        }
    });
...
PitchController.prototype.postInitialize = function() {
    ...
    const _move = event => (this.canPitchController && this.safetyLock)? this.move(event): null;
    const _release = event => (this.canPitchController && this.safetyLock)? this.pitch(this.ball, this.mouseHistory): null;
...
PitchController.prototype.move = function (_event) {    
...
PitchController.prototype.pitch = function (_ball, _mouseHistory) {
    ...
    // Actual Event Fire onto Entities
    if (this.linearVelocity.length() > this.config.minimumVelocity) {
        if (this.app.debug) console.log(data);
        _ball.fire('pitch', data);
        if(this.ui) this.ui.fire('pitch', this.ball, data);
        if(this.app.inningMaster) this.app.inningMaster.fire('pitch', this.ball, data);
        this.canPitchController = false;
    }
};

PitchController.prototype._pitch = function (_ball, _x, _y, _linearVelocity, _angularVelocity) {
    _ball.setPosition(this.view.camera.screenToWorld(_x, _y, this.depth));
    _ball.fire('pitch', { linearVelocity:_linearVelocity, angularVelocity:_angularVelocity });
};

PitchController.prototype.reset = function () {
    this.canPitchController = true;
    this.safetyLock = false;
    this.mouseHistory.reset();
    this.createBall();
};

PitchController.prototype.createBall = function (scope = this, pos = this.app.pitcher.getPosition()) {
